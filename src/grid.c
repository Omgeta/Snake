...
Point grid_spawn_food(Grid* grid) {
    ...
    unsigned short int r = rand() % queue_size(empty_points_queue);
...
Snake* grid_spawn_snake(Grid* grid) {
    ...
    unsigned short int x = rand() % (grid->row_size - PADDING) + (PADDING + 1);
    unsigned short int y = rand() % (grid->col_size - PADDING) + (PADDING + 1);
